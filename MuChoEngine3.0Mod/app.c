...
// IADVD 2023/07 BEGIN
#include "./music.h"
#define DO_SPANISH // undefined is ENGLISH
// IADVD 2023/07 END
...
unsigned char get_bit(unsigned short id)
{
	...
	// DarksideRetro - Correccion del acceso/lectura de flags en gState
	return !!(gState[id >> 3] & (1 << (id & 7)));
}
...
// DarksideRetro - Correccion del acceso/escritura de flags en gState
#define SET_BIT(x) gState[(x) >> 3] |= 1 << ((x) & 7)
    ...
    while (c)
    {
        ...
        switch (*dataptr)
        {
            ...
            case OP_CLR:
				...
				// DarksideRetro - Correccion de CLR/XOR para flags empaquetadas por bits
				gState[id >> 3] &= (1 << (id & 7))^0xff;
                break;
            case OP_XOR:
...
void reset()
{
	...
	// DarksideRetro - Correccion del reset: gState tiene 128 bytes
	for (i = 0; i < 128; i++)
        gState[i] = 0;
    for (i = 0; i < 32; i++)
        gNumber[i] = 0;
    ...
    dattrib = 000;		//divider attr 072 azul y rojo
    cls();
    clearbottom();
    go = 0xffff;
    gosub = 0xffff;
...
#ifdef DO_SPANISH
			drawstring("\x21[Fin. Pulsa espacio para empezar]", 6, 22*8);
#else
            drawstring("\x21[The end. Press space to restart]", 6, 22*8);
#endif
            drawattrib(22*8, iattrib);
