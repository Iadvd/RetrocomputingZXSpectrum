...
void mueve_bicharracos (unsigned char n_pant) {
	...
	for (i = 0; i < 3; i ++) {
		...
		if (malotes [enoffsmasi].t != 0) {
			...
			// Colisión matadora
			} else if (collide (x, y, malotes [enoffsmasi].x, malotes [enoffsmasi].y) && malotes [enoffsmasi].t < 16 && player.estado == EST_NORMAL) {
#else
			if (collide (x, y, malotes [enoffsmasi].x, malotes [enoffsmasi].y) && malotes [enoffsmasi].t < 16 && player.estado == EST_NORMAL) {
#endif			

#ifdef PLAYER_KILLS_ENEMIES
				if (y < malotes [enoffsmasi] - 14 && player.vy > 0) {
					// matar enemigo:
