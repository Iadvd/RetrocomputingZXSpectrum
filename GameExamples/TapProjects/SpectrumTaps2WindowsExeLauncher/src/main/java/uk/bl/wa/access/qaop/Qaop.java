...
public class Qaop extends Applet
    ...
    public synchronized void init() {
        ...
        //String a = param("mask");
        //if (a != null) {
            //mask = getImage(getDocumentBase(), a);
        //}
...
*/
        ...
        //a = param("rom");
        //if (a == null) {
            InputStream in = resource("spectrum.rom");
            if (in == null || tomem(s.rom48k, 0, 16384, in) != 0) {
...
//		th.setPriority(Thread.NORM_PRIORITY-1);
    ...
    protected static Hashtable param;
	...
	public static void main(String args[]) {
		...
		JavaAudioPlaySound japsoundStart = new JavaAudioPlaySound();
		
		BackgroundImageJFrame f1 = new BackgroundImageJFrame();
		f1.setResizable(false);
		f1.setTitle("This is the title of your game launcher");

		f1.setBackground(new Color(0x222222));
        f1.pack();
        f1.setSize(1024+260,567+50);
		...
		panel.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				Boolean stop_music=false;
				//System.out.println(e.getX() + "," + e.getY());
				tapname="";
				if ((e.getX()>800)&&(e.getX()<824)&&(e.getY()>239)&&(e.getY()<287)){
					make_sound=true;
					stop_music=true;
					tapname="TheHoarder_sp.tap";
				}
				if ((e.getX()>830)&&(e.getX()<855)&&(e.getY()>239)&&(e.getY()<287)){
					make_sound=true;
					stop_music=true;
					tapname="TheHoarder_en.tap";
				}
				if (tapname!=""){
					Qaop a = new Qaop();
					Frame f = new BackgroundImageJFrame();
					f.setTitle("This is the title of your game launcher");
				...
				// Info
				if ((e.getX()>769)&&(e.getX()<793)&&(e.getY()>239)&&(e.getY()<287)){
					make_sound=true;
					if (Desktop.isDesktopSupported() && 
							Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
						try{
							Desktop.getDesktop().browse(new URI("https://furilloproductions.itch.io/the-hoarder"));	
				...
				if (make_sound){
					JavaAudioPlaySound japsound = new JavaAudioPlaySound();
				}
			}
		});
		...
		f1.setVisible(true);
		// English version
		JOptionPane.showMessageDialog(null, "Put here your welcome message!");
		// Spanish version
		//JOptionPane.showMessageDialog(null, "Escribe tu mensaje de bienvenida...");
    }
...
class Loader extends Thread {
    ...
    private void do_exec() throws Exception {
        ...
        if (n.startsWith("data:")) {
			...
			raw_in = start_download(Qaop.tapname);
...
//			setPriority(Thread.NORM_PRIORITY-1);
    ...
    private InputStream start_download(String f) throws IOException {
		...
		URL u = classLoader.getResource(f);
...
class BackgroundImageJFrame extends JFrame
{
    public BackgroundImageJFrame() {
		...
		try {
			JLabel background=new JLabel(new ImageIcon(ImageIO.read(getClass().getClassLoader().getResourceAsStream("launcherbkg.png"))));
			add(background);
			setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getClassLoader().getResource("launchericon.png")));
		} catch (IOException ignored) {
		}
    }
}

class JavaAudioPlaySound
{
	public JavaAudioPlaySound() {
