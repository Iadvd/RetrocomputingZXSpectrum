...
void main_loop_background_dynamics (int x1, int x2, int y, int random, int nosound);
#endif
...
void game_ending (void) {
	...
	switch (concert_song){
		case 0:
			print_str (11, 21, 122, "SONG:   ");
			print_str (11, 22, 122, "  VOLGA ");
			// Update to screen
			sp_UpdateNow();
			
			for (i=0;i<8;i++){
				switch(in_room_is_score_n[score_set_in_pos_i_was_in_room_r[i]]){
					case 0:
							#asm
				...
				if (score_set_in_pos_i_was_in_room_r[i]==score_n_is_in_room[i]){
					// correct partial score, birdies are happy
					total_correct=total_correct+1;
					happiness=1;	
				}else{
					// correct partial score, birdies are not happy
					happiness=0;
				}
#ifndef ANDROID_TCP_IP
				main_loop_background_dynamics(4,5,1,1,1);
				main_loop_background_dynamics(9,10,1,1,1);
				main_loop_background_dynamics(5,6,7,1,1);
				main_loop_background_dynamics(9,10,7,1,1);
				sp_UpdateNow ();
#endif
			}
			break;
	...
	if (total_correct==8){
		...
		// PARTIAL SUCCESS
		print_str (6, 10, 79, "                    ");
		print_str (6, 11, 79, " STILL NOT PERFECT ");
		print_str (6, 12, 79, " BIRDY, GOOD TRY!  ");
		print_str (6, 13, 79, "                    ");
		...
		// PARTIAL SUCCESS
		print_str (6, 10, 79, "                    ");
		print_str (6, 11, 79, " YOUR MUSICAL SKILL ");
		print_str (6, 12, 79, " IS CATASTROPHIC!!! ");
		print_str (6, 13, 79, "                    ");
...
void main_loop_background_dynamics (int x1, int x2, int y, int random, int nosound) {
	...
	for (x = x1; x<x2;x++){
		r = rand() % 20;
		if (r==0){
			map_attr [x + (y << 4) - y] = comportamiento_tiles [14];
			if (happiness==1){
				map_buff [x + (y << 4) - y] = 15;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 15);
			}else{
				map_buff [x + (y << 4) - y] = 3;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 3);
			}
		}else if (r==1){
			map_attr [x + (y << 4) - y] = comportamiento_tiles [14];
			if (happiness==1){
				map_buff [x + (y << 4) - y] = 18;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 18);
			}else{
				map_buff [x + (y << 4) - y] = 13;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 13);	
			}
		}else if (r==2){
			map_attr [x + (y << 4) - y] = comportamiento_tiles [14];
			if (happiness==1){
				...
				map_buff [x + (y << 4) - y] = 14;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 14);	
			}
			...
			if (happiness==1){
				...
				map_buff [x + (y << 4) - y] = 22;
				draw_coloured_tile (VIEWPORT_X + x + x, VIEWPORT_Y + y + y, 22);	
			}
		}
	}	
...
void game_over (void) {
	...
	print_str (6, 10, 79, "                    ");
	print_str (6, 11, 79, " SORRY BIRDY! THE   ");
	print_str (6, 12, 79, " CONCERT WAS OVER   ");
	print_str (6, 13, 79, "                    ");
...
void process_tile (unsigned char x0, unsigned char y0, signed char x1, signed char y1) {
	...
	if (qtile (x0, y0) == 15 && player.keys) {
		...
		draw_coloured_tile (VIEWPORT_X + x0 + x0, VIEWPORT_Y + y0 + y0, 5);
		player.keys --;
		peta_el_beeper (8);
	}
#endif
...
void draw_scr () {
	...
	for (gpit = 0; gpit < MAX_CERROJOS; gpit ++) {
		...
		if (cerrojos [gpit].np == n_pant && !cerrojos [gpit].st) {
			gpx = cerrojos [gpit].x;
			gpy = cerrojos [gpit].y;
			draw_coloured_tile (VIEWPORT_X + gpx + gpx, VIEWPORT_Y + gpy + gpy, 5);
			gpd = 15 * gpy + gpx;
			map_attr [gpd] = 0;
			map_buff [gpd] = 0;
		}
